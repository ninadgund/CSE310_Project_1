...
//    srand(0);
    int testSize = 1000;
    TreeArray data = new tree*[testSize];
    for (int i = 0; i < testSize; i++)
    {
        data[i] = new tree;
        data[i]->freq = rand() % 10 + 1;
        data[i]->symbol = rand() % 127 + 1;
    }

    for (int i = 10; i < testSize; i += 10)
    {
        TreeArray test = new tree*[i];
        for (int j = 0; j < i; j++)
        {
            test[j] = data[j];
        }
...
//        symbolSort(test, i, mode);
        symbolSort(test, i, SortAlgo::MERGE_SORT);
...
//    /*
    cout << "Experiment: " << argv[1] << "\n";

    // Calculating expected number of bits
    int sum = 0;
    int n_sum = 0;
    for (int i = 0; i < NSYMBOLS; i++)
    {
        if (Symbols[i].freq > 0)
        {
            sum += Symbols[i].freq * strlen(Symbols[i].encoding);
            n_sum += Symbols[i].freq;
//            cout << strlen(Symbols[i].encoding) << '\n';
//            cout << Symbols[i].freq << '\t' << Symbols[i].symbol << '\t' << Symbols[i].encoding << '\n';
        }
    }
    float numberOfBits = sum;
    numberOfBits /= n_sum;

    float fixedNumberOfBits = log2(NSYMBOLS);

    cout << "n_sum: " << n_sum << "\n";
    cout << "sum: " << sum << "\n";
    cout << "numberOfBits: " << numberOfBits << "\n";
    cout << "fixedNumberOfBits: " << fixedNumberOfBits << "\n";

    float compressionRatio = (100 * (fixedNumberOfBits - numberOfBits)) / fixedNumberOfBits;
    cout << "compressionRatio: " << compressionRatio << " %\n";
//    */
